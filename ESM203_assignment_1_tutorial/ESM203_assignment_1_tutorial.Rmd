...
In this assignment, we'll be exploring surface temperatures of different neighborhoods in Los Angeles County. The first thing we need to do is load in our data, which we do with the following code. We've already run it for you, so don't worry about running it. 
...
> 2. What might be some possible explanations as to why this relationship exists? Is there something about temperature that affects NDVI or is there something about NDVI that affects temperature? Both? How/why? There could be several plausible explanations, what do you think is most likely?

> 3. Now, based upon this observed relationship, form a simple causal statement. What is causing what that underlies the observed relationship? Again, there are several possible answers.
...
> For each of the equation terms (fluxes) listed below, write one sentence defining them and explaining what they are.
...
The first thing we need to do is define our model. We do so with the code below. We've already run it for you, so don't worry about running it. 
```{r functions}
# define the stefan-boltzmann constant
sigma = 5.6704*10^-8 #W/m^2/K^4 stefan-boltzmann constant
...
> 15. If $r_W$ - the resistance to water vapor transport - is determined by the amount of vegetation, then what time of year might you expect the value of $r_W$ to be largest, and why?
...
> 25. List at least two ways in which the surface energy balance is different in the two neighborhoods discussed in the video and why that would lead to the observed difference in temperature. 

We're going to look for similar relationships to the ones discussed in the video. As an example of a plot of a relationship, remember the NDVI. v. temp plot above? Run it one more time if you need a refresher: 
