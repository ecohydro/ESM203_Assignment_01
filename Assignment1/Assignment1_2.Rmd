...
Q_av = 0.9 * (K_inc*(1-alpha) + L_inc) # W/m^2 
  ...
  Qh = 1.15*1005*(x-T_a)/r_H # sensible heat flux
  Qe = 1.15*1005/66.5*(sat_vp(x) - sat_vp(T_a))/r_w # latent heat flux
  val = - k + L + Qh + Qe
  return(val)
}
...
plot_fluxes <- function(veg){
  ...
  L_inc = 0.9 * sigma * (T_a+273.15)^4  # W/m^2, incoming Longwave
  Q_av = 0.9 * (K_inc*(1-alpha) + L_inc) # W/m^2, net radiation 
  k = Q_av
  r_w = 1/(50*veg) # scaler for veg 
  L = 0.95*sigma*(x+273.15)^4
  Qh = 1.15*1005*(x-T_a)/r_H 
  Qe = 1.15*1005/66.5*(sat_vp(x) - sat_vp(T_a))/r_w
